...
class TUNE_SE_2(object):
    ...
    def select_tuned_action(self, state, tar_replay_buffer, batch_size, tune_steps=3):
        """
        ...
        if self.total_it <= int(1e5):
            ...
            with torch.no_grad():
                ...
                if writer is not None and self.total_it % 5000 == 0:
                    ...
                    # Log classifier probabilities to check for Covariate Shift / Overconfidence
                    writer.add_scalar(
                        "debug/sas_target_prob", 
                        sas_probs[:, 1].mean(), 
                        global_step=self.total_it
                    )
                    writer.add_scalar(
                        "debug/sa_target_prob", 
                        sa_probs[:, 1].mean(), 
                        global_step=self.total_it
                    )
