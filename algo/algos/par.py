...
class PAR(object):
    def __init__(
        ...
        # aka encoder
        self.encoder = Encoder(config["state_dim"], config["action_dim"]).to(
            self.device
        )
        ...
        self.encoder_optimizer = torch.optim.Adam(
            self.encoder.parameters(), lr=config["actor_lr"]
        )
    ...
    def update_encoder(self, state_batch, action_batch, nextstate_batch, writer=None):
        ...
        if writer is not None and self.total_it % 5000 == 0:
            writer.add_scalar(
                "train/encoder loss", encoder_loss, global_step=self.total_it
            )
