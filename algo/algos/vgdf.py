...
def call_env_terminal_func(env_name: str) -> Callable:
    def is_terminal_region_for_hp(
        state: Union[np.ndarray, torch.Tensor], return_done: bool
    ) -> Union[np.array, torch.tensor]:
        if isinstance(state, np.ndarray):
            height = state[..., 0:1]
            angle = state[..., 1:2]
            not_done = (
                np.isfinite(state).all(axis=-1)[..., np.newaxis]
            ...
            angle = state[..., 1:2]
            not_done = (
                torch.isfinite(state).all(dim=-1, keepdim=True)
    ...
    def is_terminal_region_for_at(
        state: Union[np.ndarray, torch.Tensor], return_done: bool
    ) -> Union[np.array, torch.tensor]:
        if isinstance(state, np.ndarray):
            is_finite = np.isfinite(state).all(-1)[..., np.newaxis]
            is_healthy = (0.2 <= state[..., 0:1]) & (
                state[..., 0:1] <= 1.0
            )  # height - z
    ...
    def is_terminal_region_for_others(
        state: Union[np.ndarray, torch.Tensor], return_done: bool
    ) -> Union[np.array, torch.tensor]:
        not_done = True
        return not_done
    ...
    if isinstance(layer, nn.Module):
        if initializer == "xavier uniform":
            ...
            nn.init.trunc_normal_(
                layer.weight, std=1 / (2 * np.sqrt(layer.weight.shape[1]))
            )
            nn.init.constant_(layer.bias, 0)
...
class QEnsemble(Module):
    def __init__(
        ...
        self.ensemble = nn.ModuleList(
            [
                QFunction(s_dim, a_dim, hiddens, inner_nonlinear, initializer)
    ...
    def forward_single(
        self, state: torch.tensor, action: torch.tensor, index: int
    ) -> torch.tensor:
...
class SquashedGaussianPolicy(Module):
    def __init__(
        ...
        self.s_dim, self.a_dim = s_dim, a_dim
        self._model = call_mlp(
            s_dim, a_dim * 2, hidden_layers, inner_nonlinear, "Identity", initializer
        )
...
class Normalizer(nn.Module):
    ...
    def fit(self, X: torch.tensor) -> None:
        ...
        self.mean.data.copy_(torch.mean(X, axis=0, keepdims=False))
        self.std.data.copy_(torch.std(X, axis=0, keepdims=False))
...
def init_weights(m):
    def truncated_normal_init(t: nn.Module, mean: float = 0.0, std: float = 0.01):
...
class EnsembleDynamicsModel(Module):
    ...
    def train(
        ...
        for epoch in itertools.count():
            train_idx = torch.vstack(
                [
                    torch.randperm(train_inputs.shape[0])
                    for _ in range(self.network_size)
                ]
            )
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                idx = train_idx[
                    :, start_pos : min(start_pos + batch_size, train_inputs.shape[0])
                ]
                train_input = (
                    torch.from_numpy(train_inputs.cpu().numpy()[idx])
                    .float()
                    .to(self.device)
                )
                train_label = (
                    torch.from_numpy(train_labels.cpu().numpy()[idx])
                    .float()
                    .to(self.device)
                )
                losses = []
    ...
    def predict(
        ...
        if inputs.ndim == 2:
            ...
            ensemble_mean = torch.cat(
                ensemble_mean, dim=1
            )  # concat along the batch_size axis
...
class VGDF:
    def __init__(
        ...
        self.dynamics_max_epochs_since_update = config[
            "dynamics_max_epochs_since_update"
        ]
        self.dynamics_train_freq = config["dynamics_train_freq"]
        self.max_epochs_since_update_decay_interval = config[
            "max_epochs_since_update_decay_interval"
        ]
        ...
        if self.alpha is None:
            ...
            self.target_entropy = (
                ...
                else -torch.tensor(config["model_config"]["a_dim"], dtype=torch.float64)
            )
        ...
        self.optimizer_opt_policy = optim.Adam(
            self.optimistic_policy.parameters(), self.lr
        )
    ...
    def train_model(
        ...
        current_dynamics_max_epochs_since_update = max(
            0,
            self.dynamics_max_epochs_since_update
            - int(current_step / self.max_epochs_since_update_decay_interval),
        )

        s_batch, a_batch, next_s_batch, r_batch, not_done_batch = (
            tar_replay_buffer.sample(tar_replay_buffer.size)
        )
        ...
        self.loss_log["current_dynamics_max_epochs_since_update"] = (
            current_dynamics_max_epochs_since_update
        )

    def train(
        self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None
    ) -> None:
        ...
        if src_replay_buffer.size < batch_size or tar_replay_buffer.size < batch_size:
            return
        src_s, src_a, src_next_s, src_r, src_not_done = src_replay_buffer.sample(
            batch_size
        )
        tar_s, tar_a, tar_next_s, tar_r, tar_not_done = tar_replay_buffer.sample(
            batch_size
        )

        assert (
            len(src_not_done.shape)
            == len(src_r.shape)
            == len(tar_r.shape)
            == len(tar_not_done.shape)
            == 2
        )
        ...
        loss_value.backward()
        critic_total_norm = nn.utils.clip_grad_norm_(
            self.QFunction.parameters(), self.ac_gradient_clip
        )
        ...
        if self.training_count % self.training_delay == 0:
            ...
            policy_total_norm = nn.utils.clip_grad_norm_(
                self.policy.parameters(), self.ac_gradient_clip
            )
            ...
            opt_policy_total_norm = nn.utils.clip_grad_norm_(
                self.optimistic_policy.parameters(), self.ac_gradient_clip
            )
            self.optimizer_opt_policy.step()
            self.loss_log["loss_opt_policy"] = loss_opt_policy.cpu().item()
            self.loss_log["opt_policy_total_norm"] = (
                opt_policy_total_norm.detach().cpu().item()
            )

            if self.train_alpha:
                loss_alpha = (
                    -torch.exp(self.log_alpha)
                    * (new_a_log_prob.detach() + self.target_entropy)
                ).mean()
    ...
    def update_q_functions(
        ...
        src_a: torch.tensor,
        src_r: torch.tensor,
        src_not_done: torch.tensor,
        src_next_s: torch.tensor,
        tar_s: torch.tensor,
        tar_a: torch.tensor,
        tar_r: torch.tensor,
        tar_not_done: torch.tensor,
        tar_next_s: torch.tensor,
        ...
        with torch.no_grad():
            ...
            tar_value_target = tar_r + tar_not_done * self.gamma * (
                tar_next_sa_q - self.alpha * tar_next_a_logprob
            )
        tar_q1, tar_q2 = self.QFunction(tar_s, tar_a)
        tar_q_loss = F.mse_loss(tar_q1, tar_value_target) + F.mse_loss(
            tar_q2, tar_value_target
        )
        ...
        with torch.no_grad():
            ...
            src_value_target = src_r + src_not_done * self.gamma * (
                src_next_sa_q - self.alpha * src_next_a_logprob
            )
            ## 2. obtain the src value targets for comparison
            src_value_target_for_compare = (
                src_r + src_not_done * self.gamma * src_next_sa_q
            )
            ## 3. expand the dynamics model (of target domain) for TD-h value target distribution
            tar_TD_H_value_target = self._value_expansion(
                src_s, src_a, terminal_func
            )  # [ensemble_size, batch_size, 1]
            tar_TD_H_value_mean = torch.mean(
                tar_TD_H_value_target, dim=0, keepdim=False
            )  # [batch_size, 1]
            tar_TD_H_value_std = torch.std(
                tar_TD_H_value_target, dim=0, keepdim=False
            )  # [batch_size, 1]
            tar_TD_H_value_dist = torch.distributions.Normal(
                loc=tar_TD_H_value_mean, scale=tar_TD_H_value_std + 1e-8
            )
            self.loss_log["generated_value_mean"] = (
                tar_TD_H_value_mean.mean().detach().item()
            )
            self.loss_log["generated_value_std"] = (
                tar_TD_H_value_mean.std().detach().item()
            )
            self.loss_log["generated_value_max"] = (
                tar_TD_H_value_mean.max().detach().item()
            )
            self.loss_log["generated_value_min"] = (
                tar_TD_H_value_mean.min().detach().item()
            )
            ## obtain the value difference
            value_difference = torch.abs(
                tar_TD_H_value_mean - src_value_target_for_compare
            )
            self.loss_log["value_difference_mean"] = (
                value_difference.mean().detach().item()
            )
            self.loss_log["value_difference_std"] = (
                value_difference.std().detach().item()
            )
            self.loss_log["value_difference_max"] = (
                value_difference.max().detach().item()
            )
            self.loss_log["value_difference_min"] = (
                value_difference.min().detach().item()
            )
            ## obtain the likelihood
            src_value_target_in_dist_likelihood = torch.exp(
                tar_TD_H_value_dist.log_prob(src_value_target_for_compare)
            )  # [batch_size, 1]
            self.loss_log["src_value_target_likelihood_mean"] = (
                src_value_target_in_dist_likelihood.mean().detach().item()
            )
            self.loss_log["src_value_target_likelihood_std"] = (
                src_value_target_in_dist_likelihood.std().detach().item()
            )
            ...
            self.loss_log["accept_gated_ratio"] = torch.sum(
                accept_gate.int()
            ).detach().item() / np.prod(accept_gate.shape)

        # 5. obtain the loss wrt src samples
        src_q1, src_q2 = self.QFunction(src_s, src_a)
        src_q_loss = (accept_gate * (src_q1 - src_value_target) ** 2).mean() + (
            accept_gate * (src_q2 - src_value_target) ** 2
        ).mean()
    ...
    def _value_expansion(
        self, src_s: torch.tensor, src_a: torch.tensor, terminal_func: Callable
    ) -> torch.tensor:
        # imagine the next s in target domain
        dyna_pred_mean, dyna_pred_var = self.dynamics.predict(
            inputs=torch.cat([src_s, src_a], dim=-1), factor_ensemble=True
        )  # [ensemble_size, batch_size, 1 + |S|]
        dyna_pred_samples = (
            dyna_pred_mean
            + torch.ones_like(dyna_pred_var, device=self.device) * dyna_pred_var
        )
        dyna_pred_r, dyna_pred_delta_s = (
            dyna_pred_samples[:, :, :1],
            dyna_pred_samples[:, :, 1:],
        )
    ...
    def save(self, filename: str) -> None:
        ...
        torch.save(
            self.optimizer_opt_policy.state_dict(),
            filename + "_actor_optimistic_optimizer",
        )
    ...
    def load(self, filename: str) -> None:
        ...
        self.optimistic_policy.load_state_dict(
            torch.load(filename + "_actor_optimistic")
        )
        self.optimizer_opt_policy.load_state_dict(
            torch.load(filename + "_actor_optimistic_optimizer")
        )
        self.optimizer_opt_policy.load_state_dict(
            torch.load(filename + "_actor_optimistic_optimizer")
        )
