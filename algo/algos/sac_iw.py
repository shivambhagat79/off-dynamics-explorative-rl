...
class SAC_IW(object):
    ...
    def update_q_functions(
        ...
        if importance_weighting is None:
            ...
            loss = (importance_weighting * (q_1 - value_target) ** 2).mean() + (
                importance_weighting * (q_2 - value_target) ** 2
            ).mean()
        return loss
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None):
        ...
        if self.total_it <= int(1e5):
            ...
            with torch.no_grad():
                ...
                importance_weighting = torch.clip(
                    importance_weighting, min=1e-4, max=10.0
                )

                if writer is not None and self.total_it % 5000 == 0:
                    writer.add_scalar(
                        "train/importance sampling weighting",
